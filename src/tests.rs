...
fn it_should_slice_for_str() {
    ...
    assert_eq!(str_slice.take(str_slice.len()), str_slice);
}
...
fn it_should_slice_for_u8() {
    ...
    assert_eq!(
        bytes_slice.take_from(1).take(2),
        BytesSpanEx {
            offset: 1,
            line: 1,
            fragment: b"oo",
    ...
    assert_eq!(bytes_slice.take(bytes_slice.len()), bytes_slice);
}
